...
class GeminiService {
    ...
    // Dynamic Customizable Prompts
    var customExtractionPrompt: String? = null
    var customResearchPrompt: String? = null
    var customReportPrompt: String? = null
    ...
    suspend fun extractLearningMaterials(rawText: String): List<GeneratedWordItem> = withContext(Dispatchers.IO) {
        ...
        val systemInstruction = customExtractionPrompt ?: DEFAULT_EXTRACTION_PROMPT
    ...
    suspend fun researchTopicExpressions(topic: String): List<GeneratedWordItem> = withContext(Dispatchers.IO) {
        ...
        val basePrompt = customResearchPrompt ?: DEFAULT_RESEARCH_PROMPT
        val systemInstruction = basePrompt
            .replace("{topic}", topic)
            .replace("\${topic}", topic)
            .replace("\$topic", topic)
    ...
    suspend fun generateDailyReport(completedLessons: List<WordWithLogs>): String = withContext(Dispatchers.IO) {
        ...
        val systemInstruction = customReportPrompt ?: DEFAULT_REPORT_PROMPT
